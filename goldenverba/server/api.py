...
async def query(payload: QueryPayload):
    ...
    try:
        rewriting_query = await manager.rewriting_query_with_generation(payload.query)
        msg.good(f"rewriting query: '{rewriting_query}'")

        new_query = ((payload.query+' ') * 5) + rewriting_query



        chunks, context = manager.retrieve_chunks([new_query])
        ...
        msg.good(f"Succesfully processed new query: '{new_query}'")
...
async def websocket_generate_stream(websocket: WebSocket):
    ...
    while True:  # Start a loop to keep the connection alive.
        try:
            ...
            msg.good(f"Generate stream full text: '{full_text}'")
