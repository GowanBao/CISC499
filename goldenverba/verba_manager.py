...
class VerbaManager:
    ...
    def verify_variables(self) -> None:
        ...
        if os.environ.get("INTERNLM2-CHAT-7B-4BIT-HF", "") == "True":
            self.environment_variables["INTERNLM2-CHAT-7B-4BIT-HF"] = True
        else:
            self.environment_variables["INTERNLM2-CHAT-7B-4BIT-HF"] = False

        # INTERNLM2-CHAT-1_8B-HF
        if os.environ.get("INTERNLM2-CHAT-1_8B-HF", "") == "True":
            self.environment_variables["INTERNLM2-CHAT-1_8B-HF"] = True
        else:
            self.environment_variables["INTERNLM2-CHAT-1_8B-HF"] = False     
    ...
    async def generate_answer(
        self, queries: list[str], contexts: list[str], conversation: dict, generate_rewriting_query: bool = False
    ):
        semantic_query = self.embedder_manager.selected_embedder.conversation_to_query(
            queries, conversation
        )

        generate_use_cache= False
        if generate_use_cache is True:
            (
                semantic_result,
                distance,
        ...
        if semantic_result is not None:
            ...
            full_text = await self.generator_manager.selected_generator.generate(
                queries, contexts, conversation, generate_rewriting_query
            )
            if generate_use_cache is True:
    ...
    async def rewriting_query_with_generation(self, query: str) -> str:
        # Define the relative path to the rewrite_prompt.txt file, assuming it's in the project's root directory
        # prompt_file_path = "rewrite_prompt_simple.txt"
        if self.generator_manager.selected_generator.name in []:
            prompt_file_path = "rewrite_prompt.txt"
        else:
            prompt_file_path = "rewrite_prompt_simple.txt"

    
        try:
            # Attempt to read the contents of rewrite_prompt.txt
            with open(prompt_file_path, 'r', encoding='utf-8') as file:
                rewrite_prompt = file.read().strip()
        
            # If the file is successfully opened and read, send a message
            msg.good(f"Successfully read {prompt_file_path}")
        
            # Call the generate_answer function for query rewriting, assuming it supports asynchronous calls
            # and can process the combined input as input. Now passing the rewrite_prompt as part of the context
            # and setting generate_rewriting_query=True
            rewritten_query = await self.generate_answer([query], [rewrite_prompt], {}, generate_rewriting_query=True)
        
            if isinstance(rewritten_query, str):
                return rewritten_query
             #If rewritten_query is a dict, return the value of the first key
            elif isinstance(rewritten_query, dict) and "message" in rewritten_query:
                return rewritten_query["message"]
            #return rewritten_query
            
        except Exception as e:
            # Handle potential exceptions from reading the file or calling generate_answer
            msg.fail(f"Error during query rewriting: {e}")
            return query  # Return the original query in case of an exception
